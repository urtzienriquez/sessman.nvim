...
describe("model", function()
  ...
  describe("resolve() and label()", function()
    ...
    it("handles dotted project names, trailing slashes and old slash targets", function()
      vim.fn.mkdir(env.root .. "/sessman.nvim", "p")
      env.write_session(env.root .. "/sessman.nvim", "coding")
      assert.equals(env.root .. "/sessman.nvim", r("sessman.nvim:coding").project)
      assert.equals(env.root .. "/other", r(env.root .. "/other/:review").project) -- from dir completion
      local s, msg = r(env.root .. "/other/coding")
      assert.is_nil(s)
      assert.matches("write {project}:{name}", msg)
      s, msg = r(":notes")
      assert.is_nil(s)
      assert.matches("global:notes", msg)
    end)
